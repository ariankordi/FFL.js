...
// - clean up mentions and ALL REFERENCES TO "ffl"
// - should we ship THIS env texture as default? what if you dont want to use it
// - document the vertex/fragment shader code because its suspiciously simple
...
void main() {
	...
	fragEnvUV.xy = (vec2(vNormal.x, -vNormal.y) * 0.5)
...
void main() {
	...
	// This is the specular-like sphere map highlight on the surface.
	vec3 env = texture(envMap, fragEnvUV.xy).rrr; // Red channel/Luminance
	// vec3 env = texture(envMap, fragEnvUV.xy).rgb; // RGBA
	vec3 envSq = env * env; // env^2 (DXBC: (v * round(v)) / 256 ≈ v*v in float).

	vec3 envColor = clamp(diffuseColor.xyz + envSq, 0.0, 1.0);
	gl_FragColor = vec4(envColor.rgb, diffuseColor.a * opacity);
}
`;
...
/**
 * Custom THREE.ShaderMaterial styled after Wii Mii rendering.
...
class NigaoeTevMaterial extends THREE.ShaderMaterial {
	...
	constructor(options = {}) {
		...
		this.envMap = NigaoeTevMaterial.getNigaoeChannelDefaultEnvTexture();
	...
	static getNigaoeChannelDefaultEnvTexture() {
