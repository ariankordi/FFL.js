var LUTShaderMaterial = (function(three) {
...
//#region materials/LUTShaderMaterial.js
/**
...
`;
	...
	* @private
	*/
	var HermitianCurve = class HermitianCurve {
		/**
		...
		*/
		generateLUT(lutSize = 512) {
			...
			for (let i = 0; i < lutSize; i++) {
				...
				while (keyIdx < this.keys.length - 2 && pos > this.keys[keyIdx + 1].x) keyIdx++;
				const p0 = this.keys[keyIdx];
				const p1 = this.keys[keyIdx + 1];
				let t = (pos - p0.x) / (p1.x - p0.x);
				t = Number.isNaN(t) ? 0 : t;
				const y = HermitianCurve.interpolate(t, p0.y, p1.y, p0.dx * (p1.x - p0.x), p1.dx * (p1.x - p0.x));
	...
	*/
	var LUTShaderMaterial = class LUTShaderMaterial extends three.ShaderMaterial {
		/** @enum {number} */
		...
		*/
		static getLUTTextures(lutSize = 512) {
			if (LUTShaderMaterial._lutTextures) return LUTShaderMaterial._lutTextures;
			const textures = {
				specular: {},
				fresnel: {}
			};
			...
			*/
			function generateLUTTextures(lutType, target) {
				for (const key in lutType) {
					...
					target[Number(key)] = Object.assign(new three.DataTexture(lutData, lutSize, 1, r8, three.UnsignedByteType), {
		...
		*/
		constructor(options = {}) {
			...
			const uniforms = {
				...
				uDirLightColor1: { value: LUTShaderMaterial.defaultDirLightColor1 },
				uDirLightCount: { value: LUTShaderMaterial.defaultDirLightCount },
				uDirLightDirAndType0: { value: LUTShaderMaterial.defaultDirLightDirAndType0.clone() },
				uDirLightDirAndType1: { value: LUTShaderMaterial.defaultDirLightDirAndType1.clone() },
		...
		*/
		set lightEnable(value) {
			this.uniforms.uLightEnable = { value };
		}
		/**
		...
		*/
		set map(value) {
			this.uniforms.uAlbedoTexture = { value };
		}
		/**
		...
		*/
		set lightDirection(value) {
			this.uniforms.uDirLightDirAndType0 = { value };
...
//#endregion
return LUTShaderMaterial_default;
})(THREE);